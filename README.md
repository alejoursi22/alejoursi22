<!-- TOP BANNER -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:6a11cb,100:2575fc&height=120&section=header" />
</p>

## 👋 Hi there! Welcome!

I'm **Alejo Ursi**, a **Flutter Web Developer** from Buenos Aires, Argentina 🇦🇷  

💻 I work with **Flutter & Dart**, building modern, responsive and scalable web applications.  
🎨 I enjoy creating clean, user-friendly interfaces with strong UI/UX focus.  
🌱 I'm constantly learning and improving my development skills.  
🚀 Focused on building efficient solutions and growing every day as a developer.  
🛠️ Currently developing web and mobile applications that solve real-world problems.  
🎓 I will be studying a **Higher Technical Degree in Programming** at the  
**National University of Hurlingham (UNAHUR)**.

---

## 🧰 Tech Stack

### 🎨 Frontend
![Frontend](https://skillicons.dev/icons?i=flutter,figma)

### ⚙️ Backend
![Backend](https://skillicons.dev/icons?i=dart,python)

### 🗄️ Database
![Database](https://skillicons.dev/icons?i=postgresql,sqlite)

### 🔧 Versioning
![Versioning](https://skillicons.dev/icons?i=git,github)

### 🧑‍💻 Development Tools
![DevTools](https://skillicons.dev/icons?i=vscode,discord)

### ☁️ Deployment
![Deployment](https://skillicons.dev/icons?i=vercel)

### 🖥️ Operating System
![OS](https://skillicons.dev/icons?i=windows)

---

## 📫 Contact Me

<p align="center">
  <a href="https://linkedin.com">
    <img src="https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:yourmail@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-red?logo=gmail&logoColor=white"/>
  </a>
</p>

<!-- BOTTOM BANNER -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:2575fc,100:6a11cb&height=100&section=footer" />
</p>

